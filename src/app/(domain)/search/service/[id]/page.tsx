...
export default function ASDetailPage() {
  ...
  const [detailData, setDetailData] = useState<ASDetailDTO | null>(null);
    ...
    const fetchDetail = async () => {
      ...
      try {
        const res = await axios.post(
            `${process.env.NEXT_PUBLIC_API_URL}/api/v1/service/user/${id}`,
  ...
  const handleEdit = () => {
    router.push(`/search/service/${id}/edit`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/service/${id}/delete`, {
        withCredentials: true,
      });
      alert("신청이 취소되었습니다.");
      router.push("/search");
    ...
    <div className="min-h-screen bg-white text-black px-6 py-8 font-gowun">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold">에어컨 A/S 신청</h1>
        ...
        <div className="border border-gray-300 rounded-md p-6 mb-4">
          <div className="mb-4">
            ...
            <p><strong>이름:</strong> {detailData.asName || "없음"}</p>
            <p><strong>연락처:</strong> {detailData.asNumber || "없음"}</p>
            <p><strong>이메일:</strong> {detailData.asEmail || "없음"}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">제품 정보</h2>
            <p><strong>제품 종류:</strong> {detailData.productType || "없음"}</p>
            <p><strong>상세 요청사항:</strong> {detailData.asDescription || "없음"}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">방문 주소</h2>
            <p>{detailData.asAdress || "없음"}</p>
            <p>{detailData.asDetailAdress || "없음"}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">예약 정보</h2>
            <p><strong>1차 희망:</strong> {formatDateTime(detailData.asFirstReservationTime)}</p>
            <p><strong>2차 희망:</strong> {formatDateTime(detailData.asSecondReservationTime)}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">신청 일자</h2>
            <p>{formatDateTime(detailData.asStartTime)}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">진행 상태</h2>
            <p>{getInstallStatusText(detailData.asStatus || "")}</p>
          </div>
        </div>
