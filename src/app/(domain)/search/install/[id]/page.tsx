...
export default function InstallDetailPage() {
  ...
  const [detailData, setDetailData] = useState<InstallDetailDTO | null>(null);
    ...
    const fetchDetail = async () => {
      ...
      try {
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/install/user/${id}`,
  ...
  const handleEdit = () => {
    router.push(`/search/install/${id}/edit`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/install/${id}/delete`, {
        withCredentials: true,
      });
      alert("신청이 취소되었습니다.");
      router.push("/search");
    ...
    <div className="min-h-screen bg-white text-black px-6 py-8 font-gowun">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold">에어컨 설치 견적 신청</h1>
        ...
        <div className="border border-gray-300 rounded-md p-6 mb-4">
          <div className="mb-4">
            ...
            <p><strong>이름:</strong> {detailData.installName || "없음"}</p>
            <p><strong>연락처:</strong> {detailData.installPhone || "없음"}</p>
            <p><strong>이메일:</strong> {detailData.installEmail || "없음"}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">설치 주소</h2>
            <p>{detailData.installAddress || "없음"}</p>
            <p>{detailData.installDetailAddress || "없음"}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">요청 사항</h2>
            <p>{detailData.installDescription || "없음"}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">예약 정보</h2>
            <p><strong>1차 희망:</strong> {formatDateTime(detailData.reservationFirstDate)}</p>
            <p><strong>2차 희망:</strong> {formatDateTime(detailData.reservationSecondDate)}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">신청 일자</h2>
            <p>{formatDateTime(detailData.requestDate)}</p>
          </div>

          <div className="mb-4">
            <h2 className="text-lg font-semibold">진행 상태</h2>
            <p>{getInstallStatusText(detailData.installStatus || "")}</p>
          </div>
        </div>
